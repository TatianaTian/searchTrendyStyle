...
router.post("/scrape_png_check", async (req, res) => {
   {/* 修正store的格式 */}
   var short_store_url
  ...
  s3.headObject(params, function (err, metadata) {  
      if (err && err.code === 'NotFound') {  
        ...
        console.log(false)
        res.json([false, req.body.url])
      } else {  
          console.log(true)
          res.json([true, req.body.url])
...
router.post("/scrape_pdf1_check", async (req, res) => {
  {/* 修正store的格式 */}
  var short_store_url
  if (req.body.url.includes('https')){
      short_store_url = req.body.url.substring(8, )
 ...
 const params = {
    ...
    if (obj['pdf_analysis']){
      res.json([true, obj['pdf_analysis'][0].length])
    } else {
      res.json([false, req.body.url])
    }
})
...
router.post("/scrape_pdf2_check", async (req, res) => {
  {/* 修正store的格式 */}
  var short_store_url
  if (req.body.url.includes('https')){
      short_store_url = req.body.url.substring(8, )
 ...
 const params = {
    ...
    const data_analysis = await s3.getObject(params).promise()
    const obj = JSON.parse(JSON.parse(data_analysis.Body.toString()))
    console.log("obj['pdf_analysis'].length: ", obj['pdf_analysis'].length)
    if (obj['pdf_analysis'].length>6){
      const impression_url = await s3.getSignedUrl('getObject', {Bucket: BUCKET_NAME, Key: `${obj.short_store_url}/images/1.png`})
      res.json([true,obj.short_store_url, impression_url])
    } else {
      res.json([false, req.body.url])
    }
})
...
router.post("/scrape_pdf", async (req, res) => {
  console.log('Start to analyze PDF part 1, response back')
...
router.post("/fetch_objects_urls", async (req, res) => {
  console.log('start to fetch_objects_url: ', req.body)
  const store_url = req.body.store_url
  var short_store_url = ''
...
{/* 读取图片 */}
router.post("/store_img", async (req, res) => {

  console.log("req.body is: ", req.body)

  res.json("responsesss")
});
...
router.post('/image_upload', async (req, res) => {
  ...
  try{
      var s3 = new AWS.S3({accessKeyId:'AKIAVAD25KVO4Z2MJDPY', secretAccessKey:'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', region:'us-east-1'});
      //var params = {Bucket: '1688imgsearch', Key: `search_style_images/test2.jpg`, ContentType: 'image/jpeg', ACL: "public-read",};
      var params = {Bucket: '1688imgsearch', Key: `search_style_images/${img_key}`, ContentType: 'image/jpeg', ACL: "public-read",};
      s3.getSignedUrl('putObject', params, function (err, url_put) {
      //s3.getSignedUrl('upload', params, function (err, url_put) {

          console.log('PUT: Your generated pre-signed URL is', url_put);
          res.send(url_put);
