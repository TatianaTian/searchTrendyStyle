...
const analyze_pdf = async (fileContent, s3Name, short_store_url) => {
...
const analyze_pdf_p2 = async (short_store_url, num) => {
    ...
    fs.writeFileSync(`./tmp/${short_store_url}/test2.pdf`, data.Body);
    
    const options = {};
    const pdfExtract = new PDFExtract();

    const extract_words = async () => {
        const data = await pdfExtract.extract(`./tmp/${short_store_url}/test2.pdf`, options)
...
const analyzeAllPdf = async (store_url) => {
    var short_store_url
    store_url.includes('https')? short_store_url=store_url.substring(8, ) : short_store_url=store_url.substring(7, )
    short_store_url.substring(short_store_url.length-1,) === '/'? short_store_url=short_store_url.substring(0,short_store_url.length-1) : null 
    ...
    const url_to_pdf = async (store_url) => {
        ...
        try {       
            ...
            const pdf_analysis = await analyze_pdf(response_pdf.data,`${short_store_url}/original/store_front.pdf`, short_store_url)
