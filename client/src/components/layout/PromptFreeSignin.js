...
import Result_signin_free from './Result_signin_free';
...
class PromptFreeSignin extends Component {
  ...
  onSubmit = (email, password, repeatPwd) => {
    //e.preventDefault();
  ...
  onSubmitSignin = (email, password, setErr) => {
    const userData = {
      email: email,
      password: password
    };
    ...
    this.props.loginUser(userData, setErr, this.props.history)
  }
  ...
  render() {
    ...
    return ( 
        <>
          <div className="desktop-only" style={{width:width*1, marginLeft: width*0.11, marginTop: 0.08*height}}> 
            <Result_signin_free
